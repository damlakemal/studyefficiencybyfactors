...
- Excessive screen time on non-productive apps will significantly **decrease** my study time and productivity.
  ...
  **Hypothesis H1:**
  Sleep duration (especially 7.5–9h) is associated with better sleep quality, but the data does not provide evidence that it increases productivity.

  **Hypothesis H2:**
  Studying exactly at IC is associated with higher productivity in the dataset (statistically significant).

  **Hypothesis H3:**
  There is a strong positive relationship between total study time and productivity, meaning that days with more study hours tend to have higher productivity scores.

   **Hypothesis H4:**
  When caffeine intake is split into > 200 mg vs ≤ 200 mg. Productivity is significantly higher on days with more than 200 mg caffeine.

  ## Machine Learning Experiment

Supervised machine learning approach was used to model the relationship between lifestyle-related factors and studying efficiency. After preprocessing the dataset and converting all time-based features into numerical form, several regression algorithms with varying complexity levels were implemented and evaluated.
...
- Multiple Linear Regression: Models linear relationships
- Polynomial Regression (degree 2): Captures simple non-linear trends
- k-Nearest Neighbors Regression: Predicts based on similar days
- Decision Tree Regression: Learns non-linear decision rules
- Support Vector Regression (SVR): Non-linear regression with kernel methods

**Findings**

- Linear regression captured general trends but struggled with non-linear relationships.
- Polynomial regression improved performance, suggesting non-linear effects between lifestyle factors and productivity.
- k-NN and decision tree models achieved lower training error but showed signs of overfitting, consistent with the small dataset size.
- Support Vector Regression (SVR), especially after hyperparameter tuning, provided the most balanced performance between bias and variance.

These results suggest that productivity is not governed by a single dominant factor, but rather by interacting lifestyle variables. Overall, the machine learning experiment demonstrates that studying efficiency can be partially predicted using daily lifestyle-related variables such as sleep, screen time, caffeine intake, and study duration.

  ## AI Integrity Statement

In this project, AI tools and LLMs were used to support findings by generating python codes for visualizing tables and plots, computing statistical metrics. All decisions and dataset are of my own and were not generated by AI. 
